...
header__icon2.addEventListener('mouseenter',()=>{
    ...
    header__icon2.addEventListener("mousemove", e => {
        const lengthY = e.clientY - header_logo.offsetHeight/2 - header__icon2.offsetHeight/2 - header__icon2.offsetWidth/2;
        const lengthX = e.clientX - header_logo.offsetWidth/2- header__icon2.offsetWidth/2;
        header_logo.style.setProperty('--x', `${lengthX/2}px`)
        header_logo.style.setProperty('--y', `${lengthY/2}px`)
    })
})
    ...
    document.querySelectorAll('.our_works_string_hover1').forEach((el) => {
        ...
        el.addEventListener('mouseenter', () => {
            setTimeout(()=>{
                ...
                imgel_dop.style.transition='all 500ms';
            },0)
            ...
            el.addEventListener("mousemove", e => {
                const lengthY = e.clientY+ cursor.offsetWidth*2;
                const lengthX = e.clientX + cursor.offsetWidth*3;
                imgel.style.setProperty('--x', `${lengthX+20000}px`)
                imgel.style.setProperty('--y', `${lengthY}px`)

                let rotate_deg ;
                const stomatology_centr = stomatology.getBoundingClientRect().left + stomatology.offsetWidth/2;
...
//добавление к слайдеру первый слайд при мобилке
        ...
        */
        ...
        animation_with_hover_effect('animation_transition_double_effect2',
            0.1, 360, 276,
            'img/home_slider/1.jpg', 'img/home_slider/1_1.jpg', 'img/9.jpg');
